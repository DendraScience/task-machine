...
exports.TaskMachine = exports.Task = exports.NEVER_EXECUTED = void 0;
 ...
 * @module task-machine
 */
const NEVER_EXECUTED = 8640000000000000;
exports.NEVER_EXECUTED = NEVER_EXECUTED;
...
class Task {
  ...
  clear() {
    ...
    model[propKeys.executedAt] = NEVER_EXECUTED; // Invoke clear hook
  ...
  async start() {
    ...
    try {
      ...
      logger.info('Task #start, execute', {
        id,
        key
      });
      let res = await hooks.execute(model, helpers); // Abort if destroyed or preempted

      if (this.destroyed || !(model[propKeys.executedAt] === time)) return; // Process results

      if (typeof hooks.afterExecute === 'function') res = hooks.afterExecute(model, res, helpers);
      if (!res) throw Error('Result not truthy'); // Assign targets in model

      if (typeof hooks.assign === 'function') hooks.assign(model, res, helpers);
      model[propKeys.ready] = true;
      logger.info('Task #start, ready', {
        id,
        key
      });
      this.isRunning = false;
...
class TaskMachine {
  ...
  destroy() {
    ...
    logger.info('TaskMachine #destroy', {
      id
    });
    this.destroyed = true;
  ...
  set isRunning(newIsRunning) {
    ...
    logger.info('TaskMachine #isRunning(set)', {
      id,
      newIsRunning
    });
  }
